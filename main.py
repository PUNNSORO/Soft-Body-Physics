...
class rectangle:
    ...
    def simuler(self,temps_de_la_simulation,precision_dt,scale):
            ...
            for j in range(0, len(M[0])):
                if i == 0:
                    ...
                    if j == 0:
                        ...
                        liaisons[i][j][i + 1][j + 1] = [d,k,[contrainte,LIST_deformation],d,d]
        ...
        # def calculer_liaisons(M):
        #     for i in range(0, len(M)):
        #         for j in range(0, len(M[0])):
        #             if i == 0:
        #
        #                 if j == 0:
        #                     liaisons[i][j][i][j + 1][1] = raideur(i ,j + 1, i,j)
        #                     liaisons[i][j][i + 1][j + 1][1] = raideur(i + 1,j + 1, i,j)
        #                     liaisons[i][j][i + 1][j][1] = raideur(i + 1, j, i,j)
        #
        #                 if j != 0 and j != len(M[0]) - 1:
        #                     liaisons[i][j][i][j + 1][1] = raideur(i ,j + 1, i,j)
        #                     liaisons[i][j][i + 1][j + 1][1] = raideur(i + 1,j + 1, i,j)
...
care=rectangle(1/5,50,0.5,20,[1,1],10,[0.7,0.2],[[0,0,0,0]],[0,0],np.pi/6+0.001)#longueur_precision_spaciale,raideur,dissipation_lors_des_collision,mass,dimensions,alpha,position_initial,positions_de_qlq_points,vitesse_de_G,rotat
care.simuler(10000,1/(600*1),300)#temps_de_la_simulation,precision_dt,scale
